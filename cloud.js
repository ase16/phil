...
/**
 * Resize the number of instances of the given instance group
 * @param zone
 * @param instanceGroup
 * @param newSize
 * @param callback
 */
Cloud.prototype.resizeInstanceGroup = function(zone, instanceGroup, newSize, callback) {
    var params = {
        zone: zone,
        instanceGroupManager: instanceGroup,
        size: newSize
    ...
    this.compute.instanceGroupManagers.resize(params, function(err, res) {      // instanceGroupManagers.resize --> https://cloud.google.com/compute/docs/reference/latest/instanceGroupManagers/resize
        log.debug("Cloud.resizeInstanceGroup: ", err, res);
        callback(err, res);
    });
};
